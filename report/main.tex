...
\begin{itemize}[leftmargin=1.5em]
  \item \texttt{FlowMLP} (Parts 1--3): takes $(z_t, t)$, encodes $t$ with a
        128-dim sinusoidal embedding (DiT-style~\cite{peebles2023dit}), concatenates $[z_t;\, e_t] \in \mathbb{R}^{D+128}$,
...
\textbf{Sampling.} Standard flow matching uses Euler ODE integration~\cite{ma2024sit} from
...
\begin{tabular}{ll}
  ...
  Time embedding     & Sinusoidal, 128-dim (DiT-style~\cite{peebles2023dit}) \\
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Part 4.2:} MeanFlow trained at $D=32$ on all three datasets
        ...
        25K steps. Evaluated at step counts $\{1, 2, 5\}$ against the
        Part 2 baseline.
\end{itemize}
...
% =============================================================================
\subsection{v-Prediction Flow Matching at $D=2$}
...
% =============================================================================
\section{Part 2 – Flow Matching Parameterization}
% =============================================================================

\subsection{Derivation: Converting between Prediction Types}
...
% ──────────────────────────────────────────────────────────────────────────────
\subsection{Reproducing the Paper's Finding}
...
Because of this, $x$-prediction succeeds at high $D$ — its target $x$ lies on a 2D manifold, so the model only needs to learn a low-rank structure regardless of how large $D$ is~\cite{li2025denoise}. The complexity of the target does not grow with $D$.
...
% =============================================================================
\section{Part 3 – Can We Rescue v-Prediction?}
% =============================================================================
...
Consistent with~\cite{zheng2025rae} Section 4.1, which states that model width must exceed the token dimension
...
From Part 2, $v$-prediction struggles at high dimensions because the target
$v = \varepsilon - x$ is dominated by Gaussian noise — the data only lives
on a 2D manifold but the noise spans the full ambient dimension. In
Stable Diffusion 3~\cite{esser2024sd3} and FLUX~\cite{flux2025kontext}, this is not an
issue because the generative process operates on a compressed latent
space produced by a pretrained autoencoder.
The autoencoder compresses the data such that the intrinsic rank of the
latent space is close to its ambient dimension, so the noise no
longer dominates the velocity target and $v$-prediction becomes tractable.

\newpage
% =============================================================================
\section{Part 4 – One-Step Generation}
% =============================================================================
...
MeanFlow~\cite{geng2025meanflow} is a method for one-step generative modeling
that trains the model to predict mean velocity over a finite horizon.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{figures/part4_comparison_swiss_roll.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{figures/part4_comparison_gaussians.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{figures/part4_comparison_circles.png}
  \caption{Part 4.2 – Side-by-side: standard flow matching vs.\ MeanFlow on
